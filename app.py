...
def instagram():
    """User tasks"""
    try:
        link = Instagram.query.filter_by(owner_id=session["user_id"]).limit(1).all()
    except Exception as e:
        print(e)
    return render_template("instagram.html", link=link[0].link)
...
def addToLinks():
    ...
    new_todo = Instagram(link=link, owner_id=session["user_id"])
    db.session.add(new_todo)
    db.session.commit()
...
def deleteFromLinks(link_id):
    """Delete tasks"""
    link_item = Instagram.query.filter_by(id=link_id).first()
